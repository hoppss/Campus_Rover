...
geometry_msgs::Pose pev_tf_pose_;
...
void UpdatePEVPoseFromTF()
{
  ...
  pev_tf_pose_.position.x = transformStamped.transform.translation.x;
  pev_tf_pose_.position.y = transformStamped.transform.translation.y;
  pev_tf_pose_.position.z = transformStamped.transform.translation.z;
  pev_tf_pose_.orientation.x = transformStamped.transform.rotation.x;
  pev_tf_pose_.orientation.y = transformStamped.transform.rotation.y;
  pev_tf_pose_.orientation.z = transformStamped.transform.rotation.z;
  pev_tf_pose_.orientation.w = transformStamped.transform.rotation.w;
...
{
  double dist;
  dist = sqrt(pow(globle_path_.poses[globle_path_.poses.size()-1].pose.position.x - pev_tf_pose_.position.x, 2)
              + pow(globle_path_.poses[globle_path_.poses.size()-1].pose.position.y - pev_tf_pose_.position.y, 2));
...
void msgs_timerCallback(const ros::TimerEvent &event)
{
  ...
  if (status_msg_ == 1)
  {
    ROS_WARN("dp planner : Without Globle Path to follow, Waiting for the Path input");
    status_msg_ = 0;
  }
  else if (status_msg_ == 2)
  {
    ROS_WARN("dp planner : Without costmap input , Waiting for the costmap input");
    status_msg_ = 0;
  }
  else if (status_msg_ == 3)
  {
    ROS_INFO("dp planner : detect obstacle");
    status_msg_ = 0;
  }
  else if (status_msg_ == 4)
  {
    ROS_INFO("dp planner : Arrival the destination");
    status_msg_ = 0;
  }
  else if (status_msg_ == 4)
  {
    ROS_WARN("dp planner : Path switch stop");
    status_msg_ = 0;
  }
  else
  {
...
void make_path()
{
  ...
  if(first_time)
  {
    ...
    for (int pc = 0; pc < twist_set.size(); pc++)
    {
      if(twist_set[pc].angular.z == 0.0)
      {
        ...
        circle_x = 0.0;
        circle_y = twist_set[pc].angular.x;
        r = circle_y;
        angle = twist_set[pc].angular.z ;

        // x_dist_path = twist_set[pc].linear.x * dec_time;
        // rot_rad_path = twist_set[pc].angular.z * dec_time;

        //store each twist end point to vector
        //end_point.header.stamp = ros::Time::now();
        end_point.header.frame_id = robot_frame_;
        end_point.pose.position.x = circle_x + r * sin(angle);
        end_point.pose.position.y = circle_y - r * cos(angle);
        end_point.pose.position.z = 0;
        end_point.pose.orientation.x = 0;
        end_point.pose.orientation.y = 0;
        end_point.pose.orientation.z = 0;
  ...
  if(new_path_)
  {
    for (int cp = 0; cp < globle_path_.poses.size(); cp++)
    {
      ...
      dist_fp_p = sqrt((pev_tf_pose_.position.x - x_p) * (pev_tf_pose_.position.x - x_p) + (pev_tf_pose_.position.y - y_p) * (pev_tf_pose_.position.y - y_p));
      if (cp == 0)
      {
  ...
  else
  {
    ...
    while (looking_dist <= target_point_dis_)
    {
      ...
      dist_fp = sqrt(pow(new_x_fp - pev_tf_pose_.position.x, 2) + pow(new_y_fp - pev_tf_pose_.position.y, 2));
  ...
  for (int sw_pc = 0; sw_pc < twist_swap_set.size(); sw_pc++)
  {
    ...
    if(twist_swap_set[sw_pc].angular.z == 0)
    {
      ...
      p_circle_x = 0.0;
      p_circle_y = twist_swap_set[sw_pc].angular.x;
      p_r = p_circle_y;
      p_angle = twist_swap_set[sw_pc].angular.z;

      rot_rad_path_step = p_angle / step_count;
    }
    ...
    for (int tc = 0; tc < step_count; tc++)
    {
      if(twist_swap_set[sw_pc].angular.z == 0)
      {
        ...
        if (tc > 0)
        {
          ...
          x = p_circle_x + p_r * sin(angle_tw);
          y = p_circle_y - p_r * cos(angle_tw);
...
void TwistPublish(double x, double z)
{
  ...
  {
    TwistProfile(x, z);
    pub_twist.linear.x = profile_linear_x_ * ((abs(max_rot_vel_) - abs(profile_angular_z_))/abs(max_rot_vel_));
