...
private:
  ...
  campusrover_msgs::Encode encode_date_;
  ...
  ros::Publisher encoder_data_pub_, odom_pub_, battery_pub_, current_pub_, voltage_pub_;
...
void ChassisDriver::handle_speed_msg(uint8_t* buffer_data){
  ...
  encode_date_.left = rev_left_;
  encode_date_.right = rev_right_;

  encoder_data_pub_.publish(encode_date_);
...
void ChassisDriver::run(){
  ...
  if (init()){
    ...
    encoder_data_pub_ = node.advertise<campusrover_msgs::Encode>("encoder_data",1);
...
int main(int argc, char **argv){
  ros::init(argc, argv, "campusrover_driver");
