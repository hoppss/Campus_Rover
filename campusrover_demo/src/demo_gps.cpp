#include <campusrover_demo/demo_gps.hpp>
...
void getParameters(ros::NodeHandle n_private)
{
  ...
  n_private.param<bool>("pub_gps_position", pub_gps_position_, false );
...
void updatePosition(const ros::TimerEvent&)
{
  ...
  {
    try{
      transform = tfBuffer.lookupTransform( "world",
                                                   "base_link",
                                                   ros::Time(0));
    }
    catch (tf2::TransformException &ex)
    ...
    UTM_zone_ = "51R";
    gps_common::UTMtoLL(transform.transform.translation.y,
                        transform.transform.translation.x,
                        UTM_zone_,
                        gps_msg.latitude,
                        gps_msg.longitude);
    position_pub_.publish(gps_msg);
