...
bool MapTool::init()
{
  ros::init(m_Init_argc, m_pInit_argv, "pev_map_tool");
  ...
  points_sub = nh.subscribe("pev_points", 10, &MapTool::pointsCallback, this);
  path_sub = nh.subscribe("path_loader", 10, &MapTool::pathCallback, this);
  marks_pub  = nh.advertise<visualization_msgs::MarkerArray>("/pev_markers", 100);
